...
class DigitDetector:
	...
	def __init__(self):
		...
		rospy.logwarn("DigitDetector node [ONLINE]")
		...
		# Import model files
		self.p = os.path.sep.join([self.rospack.get_path('breaking_captchas')])
		self.libraryDir = os.path.join(self.p, "model")

		self.model = self.libraryDir + "/lenet_breaking_captchas.hdf5"
	...
	def shutdown(self):
		try:
			rospy.logwarn("DigitDetector node [OFFLINE]")
		finally:
...
if __name__ == '__main__':
	...
	rospy.init_node('digit_detector', anonymous=False)
	face = DigitDetector()
